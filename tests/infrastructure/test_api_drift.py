...
from agyloop.infrastructure.api.registry import REGISTERED_COMMAND_PATHS
...
def test_vertex_lane_is_registered_and_empty() -> None:
    baseline = load_baseline(lane="vertex")
    assert baseline["method_count"] == 0
    assert discover_surface(lane="vertex") == []
